...
- Classification is explicitly basis-aware.
  - `canonical_calculation` reuses `PricingCategoryResolver` and `PricingBucket::fromFacts()`: Spot maps to `spot`, quarterly MarketReset to `quarterly`, other MarketReset cadences to `market_reset`, ConsumptionEffect to `hybrid`, and Fixed to the structural term segment. It never falls back to quarterly text.
...
- `market_reset` has the generic label `Jaksoittain vaihtuva hinta`. `quarterly => Kvartaalisähkö` is both the historical and current segment for quarterly cadence.
- Old rows are not migrated, rewritten, or translated. A non-quarterly reset chart starts when rows are actually persisted under `market_reset`; observed `quarterly` and `open_ended` points do not enter it. Quarterly cadence continues its own key, with a visible gap for dates that were temporarily collected only inside the broader segment.
...
- The Quarterly SEO listing keeps its existing observed text filter and its statistics teaser always reads `quarterly`. A non-quarterly `market_reset` trend waits for a 30-day-old canonical point instead of treating differently keyed observed history as the same segment.
