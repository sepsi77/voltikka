...
A third fresh mixed 10-contract prompt-v12 production smoke test published 10/10 after 12 calls at low reasoning. Manual review found one unsafe inference in an otherwise safely blocked output: stale discount duration was used to invent a six-month phase even though `has_discount=false`. Prompt v13/validator v10 reject inactive discount timing. A local correction removed the invented phase and passed validator v10.
...
The full active-contract prompt-v14/validator-v10 backfill created 416 new interpretations and reused nine current ones. It published 414/416 after 468 model calls for $4.9846; 305 published relational pricing. Two outputs failed safely. Aalto Huoleton used `after_months=0`, which is semantically the contract start, but deterministic discount validation accepted only `contract_start`. Louna Helppo XS is an excess-use package with a monthly package fee and positive excess-use unit price; the earlier package detector handled only zero-unit-price included-energy packages. Previous valid publications remained selected for both failures.

Validator v11 accepts `after_months=0` as equivalent to `contract_start` for deterministic first-N-month discount arithmetic and recognizes strong excess-use package wording. Prompt v15 maps the excess-use package Monthly component to `flat_fee`, retains positive General as `energy_general`, adds `flat_fee_or_package` plus `fixed`, and requires a non-recurring issue code when missing package facts independently make calculation incomplete. A fresh local medium call for Louna Helppo XS passed validator v11 initially with this exact taxonomy and cost $0.0203. The stored Huoleton output passed validator v11 without a new model call.

After deployment, both failed contracts published fresh prompt-v15/validator-v11 interpretations on their initial calls for $0.0312. The complete production operation ended with all 425 active current snapshots covered by a published interpretation, 470 model calls, and $5.0158 production cost. The two failed prompt-v14 records remain unchanged for audit.

Use `openai/gpt-5.6-luna`, prompt v15, schema v3, validator v11, medium reasoning, exact production validation, and at most two automatic correction calls. Validator v11 retains prior package, warning, symmetric-evidence, structured-only, and active-discount rules. Run asynchronously and idempotently by source/prompt/validator/model/reasoning fingerprint. Persist every model attempt, evidence, usage, and validation result. Do not publish any output that still fails deterministic validation.
