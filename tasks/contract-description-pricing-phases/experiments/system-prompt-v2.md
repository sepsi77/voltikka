...
- Do not add periodic_market_reset merely because Spot settles hourly/quarter-hourly, a promotion ends, or consumption effect is calculated monthly.
...
- Metering comes primarily from structured component types: General; DayTime/NightTime => Time; SeasonalWinter/SeasonalOther => Season. Description silence is not a metering contradiction. Recurring quarterly pricing is not seasonal metering.
...
- A disclosed normal price beginning only after the first 12 months can make overall metadata incomplete, but misleading_first_12_months=not_detected.
...
ISSUE CODES
- promotion_metadata_missing: structured data lacks a disclosed introductory-to-normal component transition.
- structured_matches_intro_only: structured values match only the cheap first phase.
...
- incomplete: a required known/reverting phase lacks an amount.
