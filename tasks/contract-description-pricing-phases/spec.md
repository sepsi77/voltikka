...
Prompt/model/schema experiments are documented in `experiments/README.md` and `experiments/results.md`. The production pipeline uses GPT-5.6 Luna with prompt v8, schema v3, validator v4, low reasoning, exact automatic post-validation, and up to two bounded correction calls. Prompt v8 adds explicit pricing-type invariants and separates complete current recurring prices from exact annual calculability; validator v4 adds source-aware recurring-reset and recurring-estimate consistency checks. The first production import created 434 snapshots; its prompt v5 calls failed strict evidence validation and did not publish, which led to the aligned prompt/validator experiment workflow.
