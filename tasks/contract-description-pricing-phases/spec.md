...
Prompt/model/schema experiments are documented in `experiments/README.md` and `experiments/results.md`. The production pipeline uses GPT-5.6 Luna with prompt v12, schema v3, validator v9, low reasoning, exact automatic post-validation, and up to two bounded correction calls. Prompt v12 includes explicit pricing invariants, package detection without placeholder components, conservative Hybrid fallback, warning consistency, structured-only completeness, recurring-estimate completeness, and analysis-date cutoff rules; validator v9 adds matching deterministic checks. The first production import created 434 snapshots; its prompt v5 calls failed strict evidence validation and did not publish, which led to the aligned prompt/validator experiment workflow.
