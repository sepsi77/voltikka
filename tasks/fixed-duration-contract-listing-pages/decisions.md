...
- Use `Nousussa`, `Laskussa`, and `Vakaa` as the forecast direction labels. `Vakaata` is not a complete standalone label in this widget; the neutral headline is `Ennuste: vakaa hintataso`. Bump both the market-insight payload schema and the SEO prepared-view schema for copy-only insight changes because both caches can preserve the old payload while the data fingerprint stays unchanged.
