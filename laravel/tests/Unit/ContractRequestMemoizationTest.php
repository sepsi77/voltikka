...
class ContractRequestMemoizationTest extends TestCase
{
    public function test_contract_list_metrics_cache_reads_are_memoized_per_service_instance(): void
    {
        ...
        Cache::shouldReceive('remember')
            ...
            ->with('contract_list_metrics:v7:s12:c0r0:5000', 60 * 60 * 48, \Mockery::type(\Closure::class))
            ->andReturn($metrics);
    ...
    public function test_company_list_cache_reads_are_memoized_per_service_instance(): void
    {
        ...
        Cache::shouldReceive('remember')
            ...
            // cs12 = calculated-cost schema, lv7 = contract pricing data version,
            // c1r0 = pricing mode.
            ->with('company_list:v4:s2:cs12:lv7:c1r0:5000', 60 * 60 * 48, \Mockery::type(\Closure::class))
            ->andReturn($companies);
    ...
    public function test_contract_rankings_cache_reads_are_memoized_per_service_instance(): void
    {
        ...
        Cache::shouldReceive('remember')
            ->once()
            // s2 = ranking payload schema; cs12 = calculated-cost schema;
            // lv7 = contract pricing data version; c0r0 = pricing mode.
            ->with('contract_rankings_5000kwh:s2:cs12:lv7:c0r0', 3600, \Mockery::type(\Closure::class))
            ->andReturn($rankings);
