...
<div class="group relative w-full p-6 {{ $exceedsConsumptionLimit ? 'bg-slate-50 opacity-75' : 'bg-white' }} border border-slate-100 rounded-2xl {{ $borderWidth }} {{ $borderColorClass }} {{ $featured ? 'border-coral-200' : '' }} transition-all duration-200 hover:-translate-y-0.5 hover:shadow-card-hover">
        ...
        <div class="flex flex-wrap lg:flex-nowrap items-center gap-4 lg:gap-6 w-full lg:w-auto">
            ...
            @elseif ($generalPrice !== null)
                ...
                @if ($generalPrice == 0 && $contract->consumption_limitation_max_x_kwh_per_y > 0)
                    ...
                    <div class="text-left">
                        <div class="text-lg font-bold text-green-600 tabular-nums">
                            Sis. maksuun
                        </div>
                        <p class="text-xs text-slate-500 uppercase tracking-wide">max {{ number_format($contract->consumption_limitation_max_x_kwh_per_y, 0, ',', ' ') }} kWh/v</p>
                    </div>
    ...
    <div class="flex flex-wrap items-center gap-2 mt-4 pt-4 border-t border-slate-100">
        ...
        @if ($contract->hasActiveDiscounts())
            ...
            @endphp
            <span class="inline-flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-amber-100 to-yellow-100 text-amber-800 border border-amber-200 text-xs font-bold rounded-lg uppercase" title="Tarjoussopimus">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
