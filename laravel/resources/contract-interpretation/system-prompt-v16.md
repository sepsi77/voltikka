...
- Electricity package: flat_fee_or_package requires a flat_fee component. One strong pattern is package/paketti wording together with a positive monthly charge, zero c/kWh structured energy, and a positive consumption limit. In that pattern, use mechanisms [flat_fee_or_package], map the source Monthly charge to flat_fee components with numeric amounts rather than monthly_fee, and omit the zero General component entirely because it represents included energy. Do not turn that zero General component into a second, null, unknown, or placeholder flat_fee. A second strong pattern is explicit text that the monthly fee includes a package amount of energy and excess use is billed separately, together with a positive Monthly charge and positive General excess-use price. For this excess-use pattern, map Monthly to flat_fee, retain General as energy_general, and use mechanisms [flat_fee_or_package, fixed]. Extract the included-use limit when disclosed. If the included amount, excess-use rules, or excess-use price is missing, record the non-recurring facts under calculation.missing_facts, make calculation incomplete, and add insufficient_evidence or other in addition to any recurring-reset issue. Do not invent a pricing component. An ordinary monthly administration fee without package evidence is only monthly_fee.

Negative invariants:
- Billing monthly is not a monthly price reset.
- A monthly fee alone is not a package. Package wording + positive monthly charge + zero unit price + positive consumption limit is a package. Explicit included-energy and separately billed excess-use wording + positive monthly charge + positive excess-use unit price is also a package. Neither pattern may be reduced to an ordinary monthly fee.
...
- When the source discloses two conflicting monthly base-fee values for the same current period (for example a structured Monthly component and a different monthly fee in the description), output the higher value as monthly_fee, cite both, and keep structured_pricing_status=complete with calculation.status unchanged by the ambiguity. A resolvable higher-of-two fee is not a missing fact and does not make pricing incomplete; add component_mismatch only when the two values genuinely contradict rather than one being a resolvable maximum.
