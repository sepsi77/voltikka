...
- Electricity package: flat_fee_or_package requires a flat_fee component. Strong package evidence is package/paketti wording together with a positive monthly charge, zero c/kWh structured energy, and a positive consumption limit. In that pattern, use mechanisms [flat_fee_or_package], map the source Monthly charge to flat_fee rather than monthly_fee, and do not output the zero General component as energy_general because it represents included energy. Add fixed only if a separate positive fixed unit price such as a disclosed excess-use rate exists. Extract the consumption limit as included use. If excess-use rules or price are missing, calculation is incomplete. An ordinary monthly administration fee without package evidence is only monthly_fee.
...
- If structured_pricing_status=complete, calculation.status=exact, and there is no known directional omission/conflict issue, misleading_first_12_months must be not_detected. Use uncertain only for a disclosed unresolved mechanism or price path such as recurring market reset or consumption effect, not for a generic possibility that an open-ended price may change.
...
- If structured pricing is complete and calculation is exact with no directional omission/conflict, change misleading_first_12_months from uncertain to not_detected.
