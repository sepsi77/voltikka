...
- Scheduled compatibility permits a changed published interpretation ID only when observation key, exact source episode, snapshot and price signature still match; it retains the original stored interpretation ID and checks every other economic/source field. This permits unchanged same-episode republication without accepting changed economics.
...
- Current canonical rows take `first_observed_date` and `last_observed_date` from the pointed source-observation episode. Snapshot aggregate dates are not chronology. `source_snapshot_id` keeps immutable payload provenance, and `source_metadata.source_observation_id` identifies the exact episode.
- `price_signature` lets a source-only extension of the **same episode** continue the same price period. Forward Spot and referenced-phase `observation_key` values include the exact `source_observation_id`, so same-calendar-day A→B→A still creates two A rows. `reuseOpenPricePeriodIdentity()` requires the same observation ID, or safely adopts a tightly matched pre-rollout row with no observation ID. Never merge across recurrence.
- `lineage_key` is derived from the oldest roots in the replacement DAG. `lineage_contract_id` records the active lineage tip at collection time. `getLineageIdentity()` now delegates to `ElectricityContract::getLineageIdentitiesByContractIds()`: one canonical batch lineage read plus one adjacency read, sorted oldest roots, cycle fallback to sorted members, and the unchanged SHA-256 of roots joined with `|`. The ephemeral current supplier premium loader shares this identity, not this stored dataset or its VAT policy.
...
- Market-reset observations use the pointed episode's first-observed date as the curve vintage and delivery-period anchor. The snapshot supplies immutable payload and fingerprint provenance only. Store every available month, quarter, and year candidate.
...
Pure option validation and date-range resolution occur before any execution claim, including dry runs. Invalid historical flags/groups/ranges and unparseable dates return `INVALID` without a checkpoint. Resolve once and pass the dates into collection/backfill; retain the minimum stored price date as the historical default start.
...
`retail-premiums:cross-check` is read-only. It compares included-VAT `energy_general` term-strip observations of the **current method-version pair only** with the stored median fixed-term forecast and shows per-company medians. `--as-of` sets the collection date only. It does not rewind current contract JSON. The local `routes/console.php` runs `retail-premiums:collect --scheduled` independently in the background every five minutes from 07:15 through 12:00 Europe/Helsinki. Scheduled mode requires the current-day default scope and enforces freshness; explicit dates, selections, historical options, overwrite and dry run are rejected. The opt-in gate runs before current builders and writes no rows when same-date full contract/EEX facts, exactly one pointed source-observation episode and current publication for each active contract, current-run prior-date FI Base proof, or recent FI Base database data are absent. A gated zero-observation result fails; in scheduled mode it is terminal, not retryable waiting. `MorningFreshness/MorningConsumerExecution` shares durable effective-date exclusion across all non-dry modes. Interrupted/failed claims cannot expire into another writer, and completed scheduled days stay inert. Output transactions check the owner. Scheduled reuse of existing period rows checks identity, financial/basis/method values at storage precision, then verifies persistence and date extension. Incompatible rows require inspection, never automatic overwrite. Historical `--include-inactive` backfills bypass freshness, not writer exclusion. Manual successful runs retain immutable/overwrite semantics until scheduled completion; dry runs write neither observations nor checkpoints. See `../MorningFreshness/AGENTS.md` for waiting, deadline and failure alerts. `retail-premiums:calibrate` runs monthly on the 2nd at 08:00 Europe/Helsinki.
