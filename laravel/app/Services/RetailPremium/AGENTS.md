...
`retail-premiums:cross-check` is read-only. It compares included-VAT `energy_general` term-strip observations of the **current method-version pair only** with the stored median fixed-term forecast and shows per-company medians. `--as-of` sets the collection date only. It does not rewind current contract JSON. `routes/console.php` runs `retail-premiums:collect --require-freshness` daily at 07:15 Europe/Helsinki. The opt-in gate runs before current builders and writes no rows when same-date full contract/EEX facts, exactly one pointed source-observation episode and current publication for each active contract, current-run prior-date FI Base proof, or recent FI Base database data are absent. A gated zero-observation result is also deferred and fails. Historical `--include-inactive` backfills bypass the gate. Manual commands without the flag retain their prior behavior. `retail-premiums:calibrate` runs monthly on the 2nd at 08:00 Europe/Helsinki.
