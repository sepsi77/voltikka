...
Primary files:
- `RetailPremiumObservationService.php` builds versioned observations.
...
- Rows are immutable by default. The command skips an existing identity unless `--overwrite` is present.
