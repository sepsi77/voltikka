...
class ContractPriceCalculator
{
    ...
    public function calculate(
        ...
        if (! $hasPerKwhRates && ! $isSpotContract) {
            ...
            $monthlyDiscountSavings = $this->calculateMonthlyDiscountSavings(
                $priceComponents,
                $this->buildComponentUsageTimeline(MeteringType::General, $usage, false),
                $fixedMonthlyFee,
        ...
        $componentUsageTimeline = $this->buildComponentUsageTimeline($metering, $usage, $isSpotContract);
        $monthlyDiscountSavings = $this->calculateMonthlyDiscountSavings(
            $priceComponents,
    ...
    private function calculateSeasonalCosts(
        ...
        [$summerConsumptionFactor, $winterConsumptionFactor] = $this->getSeasonalConsumptionFactors();
    ...
    /**
     * Build a monthly kWh timeline per priced component so structured discounts can be applied safely.
     *
     * @return array<int, array<string, float>>
     */
    private function buildComponentUsageTimeline(MeteringType $metering, EnergyUsage $usage, bool $isSpotContract): array
    {
     ...
     */
    private function getSeasonalConsumptionFactors(): array
    {
        $winterMonths = 5;
        $summerMonths = 7;
