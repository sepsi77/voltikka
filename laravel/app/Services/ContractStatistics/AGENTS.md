...
- `/sahkosopimus/tilastot` caches its prepared Livewire view data per period + consumption until the next day, with cache keys versioned by the expected current basis, active annual method, and cheap source-table fingerprints. Current cache schema v16 includes the dominant-method display regime, the exact latest-day endpoint, point-marker modes, and the public reset-category sufficiency gate. The source fingerprint reads only unit rows and the active annual method, so writing inactive annual rows does not invalidate public prepared data.
...
`Jaksoittain vaihtuva hinta`. The broad segment starts only when canonical classification can prove
these mechanisms. The public statistics page hides it until its unit or annual surface has at least
30 non-null daily observations and a point on the current public endpoint. Historical `quarterly`
rows stay separate and cannot supply that threshold.
