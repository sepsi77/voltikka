...
- Shared calculated-cost schema v16 invalidates semantic caches only. It does not run a historical rebuild, rewrite snapshots/annual statistics, or replace stored method evidence. Dated annual metric keys advance at Helsinki midnight. Existing historical method rules remain dated evidence, not current annual fallbacks.
