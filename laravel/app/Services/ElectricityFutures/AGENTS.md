...
- The default full command writes a failed same-date freshness marker before date-range and instrument selection. It is ready only when this run extracted at least one prior-date FI Base point and had no fetch failures. An old FI database row cannot prove current-run acquisition; the dependent gate queries the database separately for presence and age. Targeted and dry runs do not write this global fact.
