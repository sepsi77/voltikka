...
`CurrentPriceEpisodeResolver` runs only in `CanonicalContractPricingService`, after candidates are parsed. It uses at most one snapshot query and one source-observation fallback query per batch and never runs from the calculator. It reads `contract_price_snapshots` in date order across both pricing bases and uses only the matching run at the end of that history. An old observed A price cannot hide a newer canonical B price or a later return to A. Same-date rows count as one day: known observed rates take priority over canonical rates only on that date; a null energy rate cannot override a known rate. All rows in the selected same-date evidence must match the current energy rate and fee. Conflicting rows within that evidence break the run without depending on insertion order. The existing daily continuity rule stays strict: even one missing day starts a new run, rather than inventing earlier coverage. The evidence basis describes the resolved run, not older rows; a run that uses canonical evidence has the canonical basis. A latest nonmatching or unknown date clears an older matching run and uses the source fallback below. This date is a proxy. If snapshots do not resolve it, the current source observation can supply `first_observed_at` only when its source snapshot matches the published interpretation snapshot. Missing evidence produces a typed hold-flat estimate with a missing-anchor flag. The orchestrator memoizes resolved anchors by contract, energy rate, and fee during one service lifetime, so repeated consumption calculations do not repeat the batch queries.
