...
- `ContractListCacheService::getVersion()` is the primary pricing-data invalidation signal. `contracts:fetch` bumps it after refreshing active contracts/prices, and a non-dry EEX fetch bumps it after writing prior-date FI Base data because futures now change Spot and reset annual totals
