...
- `ContractListCacheService` is request-scoped in `AppServiceProvider` and memoizes one hydrated `ContractMetricSet` per generation/evidence key. The active descriptor is reread, not memoized, so immediate invalidations reject concurrent refresh candidates. Warming releases each hydrated preset. See `Caching/AGENTS.md` for staged replacement and safety rules
- `CompanyListCacheService` consumes those list metrics. In canonical mode it accepts only listed canonical outcomes with a finite total for company membership, counts, averages, displayed prices, and price rankings; canonical-only contracts work and excluded/missing outcomes do not become zero or sentinels. Its retained generation key has its own outer payload schema, the shared calculated-cost schema, the shared active version/generation, current evidence fingerprint, and one `PricingMode` marker. The service memoizes reads per instance. Interpretation publication invalidates company output immediately. Feature-off keeps the legacy relational metrics.
...
- `DigitransitGeocodingService.php`

Important guardrails:
- Digitransit interactive address search uses one HTTP attempt with a 2-second connection timeout and a 5-second total timeout. Do not retry inside autocomplete: slow requests must return control before the PHP execution limit. Successful results keep the seven-day cache; exceptions must escape the cache callback, not become cached empty results. SolarCalculator keeps its existing unavailable notice.
