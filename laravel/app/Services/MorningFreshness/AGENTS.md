...
- Both gated jobs require the same-date ready `contract_import` checkpoint. Only retail-premium collection requires the `eex_futures` checkpoint and prior-date FI EEX Base data within the configured age. Historical-change fixed-term forecasts use no futures, so the forecast branch must not query the EEX checkpoint or futures presence/age. Contract and publication recovery checks remain unchanged.
