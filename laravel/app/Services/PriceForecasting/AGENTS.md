...
- `routes/console.php` runs `forecasting:run-fixed-contracts --require-freshness` daily at 07:30 Europe/Helsinki, after the 06:00 contract import/statistics job. The opt-in gate runs before the forecast builder. It requires same-date ready full contract/EEX checkpoints, exactly one pointed source-observation episode and a current publication for each active contract, at least one current 6/12/24 `energy_price` statistic in the request-scoped `PricingMode` basis, statistics started after the newest required publication, current-run prior-date FI Base proof, and recent FI Base data in the database. Each duration is independent. A blocked or zero-output gated run fails and writes no forecasts. Manual runs without the flag keep their prior behavior.
