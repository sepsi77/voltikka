...
- `ContractPriceStatistics::$segments` is `ContractStatisticsSegmentClassifier::SEGMENT_LABELS`. Canonical quarterly resets use `quarterly` / `Kvartaalisähkö`, so the familiar category keeps its own historical and future points. Monthly, seasonal, and other resets use `market_reset` / `Jaksoittain vaihtuva hinta`; the generic reset deep dive uses cadence-neutral copy. The non-quarterly segment's comparable public history restarts on 2026-08-10 and must have at least 30 non-null daily observations, including the current public endpoint, before it appears in the unit table, annual table, or deep dive. The quarterly deep dive can show at least 30 dated observations when no current row exists, but labels its last date and never calls the old contract count current. Current summary tables still require an endpoint row. A missing full chart period gets one null point so old and new quarterly runs do not connect across a collection gap
