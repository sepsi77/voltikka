...
- The forecast lead summarizes all available median directions as down, up, stable, or mixed. Every visible duration row shows its median change and `ennusteen luotettavuus` as `hyvä`, `kohtalainen`, or `heikko`. The copy always says that the forecast is not a promise.
