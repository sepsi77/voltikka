...
- The page follows one household decision sequence: conclusion, concept, annual cost, current energy price, duration interpretation, history, forecast, checklist, and offer links. The conclusion derives its cheaper/costlier/equal wording and small-difference judgment from the payload. It must not return to equal-weight data sections or internal method language.
- Page copy keeps contract duration separate from pricing method. The canonical current unit comparison uses the open-ended segment's published current fixed rate and fully fixed 6/12/24 month segments on one common date. It never implies that an open-ended seller rate stays fixed for the future. Feature-off mode fails closed. The lowest and highest fixed-duration statements come from the payload ranking, not fixed copy.
- The main answer uses the 5,000 kWh annual median difference between an open-ended fixed-price product and a fully fixed 12-month product. It includes energy and monthly contract fees, excludes transfer, and labels the open-ended 12-month total as an estimate because its seller can change the price under the terms. A difference is small when it is at most 5 €/month or 5% of the lower median. In that close-price state, 12 months is presented as a sensible one-year certainty choice, not as a universal recommendation.
- Main annual, current-price, and forecast rows fit without horizontal scrolling. p20, p80, counts, exact forecast target dates, method detail, and complete weekly values stay in native disclosures. The first disclosure explains p20–p80 as the middle 60% of listed prices.
- History uses fixed 6/12/24 only in three server-rendered weekly SVG small multiples on one shared clean-number scale. Each chart has a fixed-width HTML y-axis beside its flexible SVG plot, 14px c/kWh tick labels aligned to SVG grid lines, a visible HTML legend, first/last dates, and a generated rose/fell/nearly-unchanged sentence. Open-ended stays out because observed historical classification can drift.
- The forecast lead summarizes all available median directions as down, up, stable, or mixed. Every visible duration row shows its median arrow/change and confidence. The copy always says that the forecast is not a promise.
