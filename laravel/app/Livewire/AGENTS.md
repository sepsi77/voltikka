...
- Current `fixed_term_futures_adjusted_v1` copy/schema explain contract-price development plus wholesale prices in short household Finnish. The headline asks where fixed-term prices are going; futures are explained once as future-month wholesale prices. Full historical mean plus fixed-basket seven-day ridge adjustment stays in service documentation, not the main UI. Legacy financial diagnostics stay hidden, not cast to zero. Crossing quantiles retain their lanes and explicit warning; the article keeps its ordered guard. See `../../../tasks/futures-adjusted-forecast-release/`.
- comparison-page forecast teasers use the same model scope through `ContractMarketInsightService`
- the local qualified outlook uses shared `PriceForecasting/ForecastOutlook` saved direction categories, not legacy consumer_signal. Slight moves mean approximately unchanged; unknown is unavailable. A complete three-term agreement is required for the overall direction; partial data and mixed directions have distinct copy. The page and article use the configured horizon to select rows and show stored issue/target dates. Public text gives expectations, not lock/wait instructions. The forecast page has one visible uncertainty notice: `Ennuste voi muuttua markkinatilanteen mukana.` It keeps saved dates/horizon and current price comparison. History-count confidence describes `Ennusteen tietopohja`, not accuracy/probability; p20/p80 describe contract-price distributions, not uncertainty intervals. Layout, controls and Preferred Sources are unchanged. This local release still requires approval.
...
- `getDailyStatsProperty()` keeps an explicit request/job-scoped collection cache and selects only the columns used by the view-data builder. It hydrates all `unit_statistics_v1` rows. It hydrates active-method `annual_cost` rows only for the selected consumption. Queued warmers instantiate the component directly, so do not rely only on Livewire computed-property memoization for this table read.
...
- cache invalidation is automatic through cheap `contract_price_daily_statistics` / `contract_price_snapshots` / spot-price max-date/update fingerprints, so daily imports/backfills should not need manual page-cache clearing. The Spot fingerprint includes daily rows plus legacy and corrected local rolling 30/365 rows; hour and price sums detect same-second coverage/level corrections without new dates or rows
...
- forward rows store `pricing_basis=canonical_calculation`; historical backfill and feature-off rows store `observed_seller_data`. Current unit panels and the endpoint date require request-scoped `PricingMode::expectedContractPriceBasis()`. Annual panels have a separate latest eligible active-method endpoint, bounded by the unit date and requiring the expected basis or `mixed_evidence`. Retained annual data behind unit collection is explicitly dated historical data, not today's comparison. This compares data endpoints, not clock age; ordinary yesterday data is not broadly reclassified. A stale canonical AsOf row cannot survive a feature-off recalculation into public output. Historical rows stay method-filtered and date-scoped. The CSV exports all method/provenance fields and marks the active annual version. Prepared view-data cache schema is v21; its key and source fingerprint include the active annual method and seller-set index rows. It retains the coral seller-set index line and adds the separate annual endpoint. Both AsOf versions use the same sample and regime rules, with the actual version in the display key. Current collection writes v2, and public `annual_cost_as_of_v2` has been active since the explicitly approved activation on 2026-09-12. Historical v2 was applied for 233 evidence dates from 2026-01-21 through 2026-09-11 (no evidence date on February 12). V1 remains retained at 2026-09-11 with its snapshot joins preserved; historical snapshots and price components are unchanged. A separately approved switchback restores dated retained v1, not current v1 recomputation. See `../../../tasks/annual-statistics-v2-rollout/apply-results.md` for proof
- the page leads with the persisted seller-set energy-price index when canonical rows exist. The chart shows overall, fixed-term, open-ended, and market-reset direct General rates in c/kWh, keeps Hybrid base price separate, follows the selected daily/weekly/monthly period, and inserts null periods for missing collection dates. Its exact 30-day change and family weights are independent of the annual-consumption selector
...
- `ContractPriceStatistics::$segments` is `ContractStatisticsSegmentClassifier::SEGMENT_LABELS`. Canonical quarterly resets use `quarterly` / `Kvartaalisähkö`, so the familiar category keeps its own historical and future points. Monthly, seasonal, and other resets use `market_reset` / `Jaksoittain vaihtuva hinta`; the generic reset deep dive uses cadence-neutral copy. The non-quarterly segment's comparable public history restarts on 2026-08-10 and must have at least 30 non-null daily observations, including the current public endpoint, before it appears in the unit table, annual table, or deep dive. The quarterly deep dive and both summary tables can show at least 30 dated observations when no current row exists. Every dated Quarterly surface labels its last observation below the category, and the deep dive never calls the old contract count current. Other categories still require an endpoint row. A missing full chart period gets one null point so old and new quarterly runs do not connect across a collection gap
...
- The broad answer uses a fully fixed 12-month contract as the benchmark. Visible Finnish calls it `täysin kiinteähintainen 12 kuukauden sopimus` on first mention and `kiinteä 12 kuukauden sopimus` later. It gives separate plain-Finnish answers against `pörssisähkö`, an open-ended fixed-price contract, `jaksoittain vaihtuva hinta`, `kvartaalisähkö`, and `kulutusvaikutus`. The conclusion can name different lower mediaanikustannus outcomes in different complete comparisons and must keep the price-certainty trade-off visible.
- Annual estimates use 5,000 kWh for the next 12 months, include energy and monthly contract fees, and exclude transfer with the exact disclosure `Sähkönsiirto ei kuulu vertailuun.` Spot is a forward estimate; an open-ended seller can change its price; later reset periods are estimates. Annual group copy uses `vuosikustannus` and `mediaanikustannus`. The 6-month value is a 12-month comparison equivalent, and the 24-month row covers only its next 12 months.
- The article deliberately compares base-price totals when a consumption effect is unknown. Hybrid gets its stored base-price annual distribution, and a market-reset or quarterly aggregate can retain `hybrid_base_only` contributors. Every such row, chart value, and verdict must state that the consumption effect is assumed to be zero and the actual cost can be higher or lower. This article-only presentation rule does not change list ranking or canonical calculator output. A clean market-reset sample below 10 still needs explicit small-sample caution.
- Every data section is conclusion-first: state the answer or unavailable verdict, then quantify it, then explain method and limits. A short verdict must name both compared contract types; never write an unscoped `Hintaero on pieni.` For a small difference, name the pair before the euro amounts. Do not make readers infer the point from details.
- The flat `Hinta ja hintariski rinnakkain` illustration uses the same annual rows as the main chart and adds a three-level editorial price-predictability scale. Consumption effect is the middle tier: its base price is known even though the customer-specific adjustment is not. Do not invent a typical 0.3 c/kWh adjustment; only 13 of 45 active household/Both effect disclosures in the 2026-08-11 snapshot have any numeric bound, only two have an expected value, and one is explicitly uncapped. Its Hybrid and affected reset values are visibly base-price comparisons with the effect excluded. The copy must say that more price risk can create a saving opportunity but does not guarantee a lower price; the current aggregate data is not monotonic.
- Page copy keeps contract duration separate from pricing method. The canonical current unit comparison uses the open-ended segment's published current fixed rate and fully fixed 6/12/24 month segments on one common date. It never implies that an open-ended seller rate stays fixed for the future. Feature-off mode fails closed. The three-duration `fixedTermComparison()` consumer stays unchanged.
- The annual and history graphics use Chart.js through the established article pattern: the global CDN script, fixed-height responsive canvases, visible HTML legends, Finnish formatting, destroy-before-recreate lifecycle, and initialization on both `DOMContentLoaded` and `livewire:navigated`. Chart animation is disabled so the evidence paints immediately and never creates reduced-motion concerns. The annual chart is horizontal and contains complete rows only. The one history chart shows only weekly 6/12/24 medians, without overlapping p20–p80 bands.
- Both charts have a visible takeaway, `aria-describedby`, and native disclosure tables with captions and row/column scopes. p20, p80, counts, exact forecast target dates, and complete weekly values stay in disclosures.
- The forecast lead uses shared saved direction categories: all three complete terms must agree for up/down/stable; missing terms give incomplete, and differing categories give mixed. Unknown directions are unavailable. Every visible term shows its saved horizon/target date and qualified outlook. `Ennusteen tietopohja` describes long, moderate or limited comparable price history, not accuracy or probability. The copy always says that the forecast is not a promise.
- `dateModified` is the fixed editorial review date 2026-08-31. The newest stored market or forecast date is visible as a separate market-data date and never changes the editorial review date. The plain-language HTML description and Article schema title/headline omit a year so they cannot become stale.
- Do not add claims about moving, cancellation fees, continuation, or long-term prices. The checklist tells readers to inspect the seller's terms without predicting what those terms contain.
...
2. **`#hintavertailu` — "{yhtiö}: hinnat markkinaan verrattuna."** Range rows per
   ...
   a payload-marked historical fallback with explicit dated copy. Both AsOf versions select the
   exact active method. A canonical annual pair behind the same-basis unit endpoint is also a
   historical retained comparison, suppresses current Spot benchmarks, and uses dated copy.
   It never calls either fallback today's price. Company comparison payload cache schema is v11.
...
- the page also renders a `sähkön hinta laskuri` section when `contract_price_daily_statistics` data exists. Every contract type uses stored `annual_cost` p20/median/p80 rows with the existing interpolation/nearest-reference behavior. Both the latest-date query and row query explicitly filter the active annual method, so inactive-method rows cannot change public output. Public v2 has been active since 2026-09-12; retained v1 is isolated from these reads. The date and rows use `PricingMode::expectedContractPriceBasis()`: canonical mode requires `canonical_calculation`, and feature-off requires `observed_seller_data`, with no newer wrong-basis fallback. Retained active annual data keeps its own displayed date while unit collection advances. It never rebuilds a public annual total from unit price + monthly fee. If annual rows are missing, that type is unavailable; this keeps canonical-only and package totals while preventing relational fallback.
...
- cache keys include route/filter context plus `ContractPageCacheVersion::hash()` so contract imports and source-table changes bust stale payloads. The current outer schemas are `contracts-list:view-data:v6` and `seo-contracts-list:view-data:v10`; both keys include forecast model and horizon to prevent stale forecast copy; the SEO key includes exact `fixed_time_range`. The SEO version includes the fixed-term decision-guide payloads
...
- The guide stays server-rendered and has one short duration note, one shared checklist, and three visible FAQs per variant. Only the exact 6-month guide calculates an illustrative end date from `CarbonImmutable::now('Europe/Helsinki')->addMonthsNoOverflow(6)`. November through March end dates get a winter-renewal price warning; every end date tells the visitor to compare again 1–2 months before expiry and states that the order confirmation start date controls the actual end date. The 6-month card uses a 12-month comparison figure although the contract ends after 6 months. The 24-month copy states that the comparison covers 12 months while the commitment lasts 24 months. Fixed terms normally bind for the term; seller terms govern moves and exceptional termination.
- Contract comparison hero market-insight pills are intentionally small and must not push results down. They use cached precomputed statistics/forecast payloads from `ContractMarketInsightService`; do not calculate contract prices or scan raw `price_components` during page requests. The broad aggregate pill is canonical-only and reads the persisted consumption-independent seller-set energy-price index. Its 30-day change requires the exact prior calendar date, and its footer names eligible offers and suppliers. It never falls back to annual cost or observed data. Segment-specific and exact-duration pages keep their existing strict annual/unit trend rules; exact-duration pages pass their own 6/12/24 month forecast duration. Cache keys and fingerprints vary by flag, basis, exact duration, and index endpoint. SEO listing prepared view data uses schema v10 for the qualified fixed-term outlook. Guide forecast copy gives market expectations and dated uncertainty, never lock/wait or duration advice; euro changes remain annual rate equivalents, not savings from waiting.
