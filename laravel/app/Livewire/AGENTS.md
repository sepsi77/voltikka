...
- Contract comparison hero market-insight pills are intentionally small and must not push results down. They use cached precomputed statistics/forecast payloads from `ContractMarketInsightService`; do not calculate contract prices or scan raw `price_components` for these pills during page requests. Their latest point uses the basis expected by the canonical flag. In canonical mode the 30-day comparison can use an older dated observed point, and its visible supporting copy identifies that provenance. The non-quarterly `market_reset` segment is the exception: it waits for an older canonical point instead of treating historical `quarterly`/`open_ended` rows as the same segment. The Quarterly SEO page always reads `quarterly`. Exact-duration pages pass their own 6/12/24 month forecast duration and segment; broad fixed-term behavior keeps its 12-month default. Cache keys and fingerprints vary by flag, basis, and exact duration.
