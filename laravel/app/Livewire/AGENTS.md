...
- current `fixed_term_historical_change_v1` copy/schema explain current price plus equal-weight completed historical changes, independently by term and quantile, with overlapping periods and turning-point lag. No futures input is used. The page omits nullable legacy fair-price/gap/futures diagnostics instead of casting them to zero. Crossing quantiles remain in their original lanes with an explicit warning that they are not an ordered distribution; the article keeps its existing ordered guard. See `../../../tasks/historical-change-forecast-release/`.
- comparison-page forecast teasers use the same model scope through `ContractMarketInsightService`
- the local qualified outlook uses shared `PriceForecasting/ForecastOutlook` saved direction categories, not legacy consumer_signal. Slight moves mean approximately unchanged; unknown is unavailable. A complete three-term agreement is required for the overall direction; partial data and mixed directions have distinct copy. The page and article use the configured horizon to select rows and show stored issue/target dates. Public text gives expectations, not lock/wait instructions. `Suuntaa antava arvio, ei varma hintakehitys.` stays visible near the lead and term outlooks. History-count confidence describes `Ennusteen tietopohja`, not measured accuracy/probability; p20/p80 describe offer-price distributions, not uncertainty intervals. Layout and Preferred Sources are unchanged. This code is not released.
