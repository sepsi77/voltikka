...
- the job resolves its stored schema/prompt/validator tuple before input and model work. Current config changes cannot change a pending job's schema or validation. Unsupported tuples fail closed without a client call; stored version fields stay unchanged. Explicit V5 analyses use real V5 assets and full V18 source proof, including at locked publication. This does not change the active V4 default
...
- verifies episode ownership, exact manifest fingerprint, semantic input evidence fingerprint, episode fingerprint, analysis fingerprint, and every configured version before an HTTP call. Schema/prompt/validator/parser versions use the pinned historical config subtree, not current defaults. The runner uses that historical profile for real prompt assets and deterministic validation
- requires `ContractInterpretationValidator`, the historical-only backcast restriction validator, and `CanonicalPricingParser` success with explicit `withEnergyRules: false`. Backcast text can classify a stable mechanism, but numeric billed and package facts must match one cited exact structured component by shared canonical type, unit, role, and scoped discount timing. Recurring-period dates and numeric consumption effects stay null, duplicate billed components fail, and backcast evidence can never produce `detected` deception
...
- Supervisor runs the shared worker with `php /app/artisan queue:work --queue=default,historical-interpretation --timeout=1020 --tries=3 --max-time=3600`. Laravel checks the maximum time between jobs. It does not stop the current job. Supervisor then starts a fresh worker. This hourly recycle limits memory that a long-lived worker keeps between jobs.
