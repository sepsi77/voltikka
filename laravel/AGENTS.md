...
- prepared page entries expire at tomorrow and also bust through `ContractPageCacheVersion`, which includes the active `ContractListCacheService` version, current active source/publication identities, and cheap source-table aggregates. Annual list/company prices do not expire at midnight or after 48 hours; rankings reuse those retained prices
- after a complete import and successful required statistics, `ContractPostImportCoordinator` builds and verifies all eight preset payloads plus company/5,000 from that candidate's metrics before one atomic activation. A refresh failure is the required `price_cache_refresh` stage and leaves the prior pointer unchanged. Partial imports and statistics failures do not start replacement
- the coordinator forgets only the owned sitemap key, never flushes the cache store or truncates its table. Tracked prior-generation payloads receive a one-hour grace period after activation. `contracts:cleanup-price-cache` runs every five minutes and physically deletes at most 100 due tracked payload keys across at most 20 generations, with durable manifests/index retry state and an active-generation check under the transition lock. A TTL alone cannot release abandoned MySQL cache rows; failed cleanup cannot undo activation
- `cache:reclaim-expired-files` separately reclaims expired default-FileStore array/object payload bytes. It is dry-run by default; the per-instance minute schedule uses `--apply --scheduled` with a 30-second scan budget and rotating hash shards. It never flushes, deserializes, or unlinks files: locked truncation retains zero-byte inodes for waiting writers. Live/forever values and scalar lock owners stay unchanged. Other stores are no-ops. See `app/Services/Caching/AGENTS.md` for the header-only validation boundary and manual catch-up.
- `contracts:warm-cache --refresh` uses this same staged path. Interpretation, EEX, and explicit `bumpVersion()` calls still invalidate immediately through the shared transition lock and reject concurrent candidates. Shared canonical cache reads fail closed for changed or mismatched source/publication identity. They never require `relational_pricing_published`: canonical corrected pricing can remain valid when relational writes are blocked. The typed canonical outcome decides listability. Direct calculations remain current, not a site-wide frozen database snapshot. See `app/Services/Caching/AGENTS.md`
- only verified typed evidence/generation conflicts retry, with two total cold-read or full-candidate attempts and reset request-local calculation state. Company reads share the budget with their list dependency. Failed candidates require durable retirement; no retry reverses an independent invalidation. Exhausted HTTP conflicts render plain Finnish 503 with Retry-After and no-store, and only that class skips automatic reporting. Other exceptions keep normal failure/reporting behavior. See `app/Services/Caching/AGENTS.md`
...
- `CanonicalPricing/PricingMode` snapshots canonical and reset-shift flags once per request/command and owns the expected statistics basis and cache marker. All calculated-cost-dependent caches include `CalculatedCostPayloadSchema::VERSION`; their outer wrapper versions remain independent. Schema v16 invalidates annual-pricing semantic caches once, without rewriting stored interpretations, historical statistics, or old method evidence. A cache schema bump never rebuilds history; when it moves annual totals, plan the history rebuild separately
- annual estimates keep known phases exact and use an explicit latest applicable price or disclosed normal continuation for unknown segments. Unknown future prices alone do not exclude a contract. Gaps never become free energy or extended promo savings. Short terms retain real-term annualization and Hybrids retain all base phases without the consumption effect. No-overflow anniversaries govern relative dates, ordinary fees, and monthly bins; package fees and allowances stay calendar-month scoped. One default flat consumption profile applies across tariffs, with explicit heating/cooling shape. Reset/supplier annual equivalents reconcile with billed energy and costed kWh. See `app/Services/CanonicalPricing/AGENTS.md`
- canonical prices use Household/Both/null VAT-inclusive and Company VAT-excluded conventions. Explicit component VAT is normalized in calculation copies, unknown VAT assumes that target, and inclusive market curves convert once before offsets. Typed tax/estimate facts control copy; LLM summaries are never rendered
...
- `contract_price_annual_costs` — versioned annual-only rows by date, contract, consumption, and method; current canonical collection writes public v2 beside retained rollback v1.
...
php artisan contracts:rebuild-annual-cost-statistics --from=2025-01-01 --to=2026-04-29 --method=annual_cost_as_of_v2 --baseline=annual_cost_as_of_v1
# Planned full-date apply only after explicit approval and a verified full backup:
php artisan contracts:rebuild-annual-cost-statistics --date=2026-04-29 --method=annual_cost_as_of_v2 --baseline=annual_cost_as_of_v1 --apply
```
...
- `/sahkosopimus/tilastot` serves cached prepared view data per period + consumption. Its v21 key and source fingerprint include the configured active annual method. Production has used `annual_cost_as_of_v2` since the explicitly approved activation on 2026-09-12. Historical v2 was applied for 233 evidence dates from 2026-01-21 through 2026-09-11 (no evidence date on February 12). V1 remains retained at 2026-09-11 with its snapshot joins preserved; historical snapshots and price components are unchanged. See `../tasks/annual-statistics-v2-rollout/apply-results.md` for proof. Current canonical collection selects v3 only when active-method configuration is exactly `annual_cost_as_of_v3`; all other configurations keep the existing v2 producer. The backup, reviewed-apply, and separate method-switch approval safeguards in `../tasks/annual-statistics-correction/rollout.md` still apply; a rollback selects dated retained v1. Deployment never applies a historical rebuild automatically. A rebuild is nevertheless a required, planned step of each annual method change: see "History follows the current method" in `app/Services/ContractStatistics/AGENTS.md`
...
- Production containers start `php artisan queue:work --queue=default,historical-interpretation --timeout=1020 --tries=3 --max-time=3600` through root `supervisord.conf`; database queue `retry_after` has a code-enforced minimum of 1,050 seconds. A lower environment value cannot release a job while it still runs. Laravel checks `--max-time` between jobs, so the current job finishes before Supervisor starts a fresh worker. The default queue has priority, and historical interpretation uses its isolated queue. Historical model calls have a separate 300-second total timeout and one HTTP attempt. One initial call plus two repair calls stays below the 1,000-second historical job timeout. Other job-level timeouts stay unchanged. Queued cache warmers and contract interpretation depend on that worker running.
...
- the version-aware AsOf historical core uses the union of exact-date snapshot and component contract identities. A component-only identity has no safe historical classification, so it yields exactly three typed `missing_historical_snapshot_identity` exclusions under `unclassified`; snapshot-only canonical evidence remains eligible. It first uses one unambiguous covering source snapshot and a parser-valid interpretation completed by that date. Approved explicit v3 reconstruction instead selects the latest exact-source interpretation that passes full stored-profile validation against that payload and target date, independent of completion relative to target. Unsupported calendar/relative-date proof stays unresolved; rejected candidate IDs and target-validation flags are retained. Selected-time ties fail closed. Typed completion/retrospective provenance and explicit legacy NULL observation-binding flags remain. This supersedes the processing-dependent timely/first-later rule. It never imports later seller facts/prices, current peers, a later different source or future curves; missing output stays unresolved. V1/v2 and strict resolver defaults are unchanged. Explicit v3 uses dated left-censored exact-contract anchors and exact-date supplier/reset premium evidence with shared Current candidate/calculation semantics with explicit historical dates. V1/v2 stay Historical. No current pointer, company or premium-store reads supply v3 evidence. Exact historical payload campaign proof is shared through a pure extractor; V5 source rules fail closed until dated validation exists. Older absent donors and dated replacement lineage remain unsupported. Local coverage/median review passes for 242 dates at all three consumptions, with July's processing seam repaired and 11 documented real reference/membership transitions. Production approvals remain pending; no full-store amount equality or forecast improvement is claimed. V3 compares full dated energy signatures independent of fees, with no raw fallback over missing/invalid covering source output; see the statistics context for the exact boundaries. Only when no source observation covers the date can one current-builder dedicated historical episode supply canonical data. The episode must contain the exact target snapshot and sorted component composite identities, and exactly one validated current-contract historical analysis must pass a fresh parser check. It batches components, chronology, both interpretation stores, exact-target Spot assumptions, and supplier episode anchors. It does not read active contracts, current canonical columns, prose, or current interpretation pointers. `contracts:rebuild-annual-cost-statistics` selects snapshot/component dates plus retained baseline dates. It defaults to v2 preview against the stored active method and compares before any write, including baseline=target. Partial previews use the same deterministic contract universe for both sides; output reports matched/new/lost aggregates and contract-consumption coverage. Apply accepts v2 or explicit v3, replaces only that method, and rejects partial selectors, empty evidence, or incomplete three-consumption identity sets before replacement. Command defaults and the active/public method stay v2; the current producer selects v3 only under explicit active-v3 configuration. No automatic rebuild is added. V1 hold branches remain diagnostic, not frozen original mathematics. V2 uses canonical reset estimates and date-safe supplier seasonal inputs. Old null annual masks are recoverable only from selected immutable source payloads with proven bounds or explicit unrestricted bounds; unknown retrospective limits stay unavailable. A Company-only selected source produces a household conflict. Current contract fields cannot supply these dated proofs. Canonical current daily calculation instead adapts the exact already-calculated public canonical outcomes after snapshots exist, with one batched current-pointer provenance query and no `price_components` query, and writes the selected v2/v3 method. The factory defaults to v2 and accepts explicit v2/v3 only. The service captures v3 only for exact active-v3 configuration, otherwise v2, and shares that method with factory and writer inside the same date transaction. Inactive v1/v2 annual and aggregate financial rows remain byte-identical. The annual table has no compatibility-snapshot foreign key, but replacement can leave old snapshot IDs in provenance stale and remove company date/contract joins; avoid overwriting the last retained v1 date. Feature-off and historical observed calculations do not run the current adapter. Public readers remain method-isolated
- `ContractStatisticsSegmentClassifier` owns the one basis-aware segment rule and label map. Canonical card facts map quarterly resets to `quarterly` (`Kvartaalisähkö`) and monthly/seasonal/other resets to `market_reset` (`Jaksoittain vaihtuva hinta`), with Spot and reset-over-Hybrid precedence from `PricingBucket`. Observed rows keep the old text-first `quarterly` rule and are never rewritten
- all public annual-cost consumers explicitly select the configured active annual method. `annual_cost_as_of_v2` has been active in production since 2026-09-12. Unit consumers explicitly select `unit_statistics_v1`; mixed queries use the model scope that combines those two branches. Canonical flag on still requires `canonical_calculation` for current unit data, while feature-off requires `observed_seller_data`. Annual panels use their own latest eligible active-method endpoint, no later than the unit endpoint, with the expected basis or `mixed_evidence`. Older retained annual values show their date and historical copy; feature-off cannot expose stale canonical AsOf rows. Stored compatibility stays strict for audit. Public aggregate median charts include the actual AsOf version in their display key and derive a display regime from the dominant estimate method in valid AsOf basis counts, so minority changes do not split a line but dominant transitions still do. Weekly/monthly statistics series append the latest daily median at its exact date and show point markers. Daily charts mark only each series' latest non-null point, so a current isolated value remains visible without covering the dense history in dots. Company comparison branches by the active method: legacy keeps snapshot annual columns, while all AsOf versions read seller totals only from exact-active-method `contract_price_annual_costs` and require same-date market rows. A retained canonical company pair behind unit collection uses historical dated copy and no current Spot benchmarks. Both consumption-calculator queries also filter the active annual method
- one pricing basis owns each newly calculated date: inside the calculation transaction, the target date loses opposite-basis snapshots and the run's own prior snapshots before aggregates are rebuilt; this removes stale canonical exclusions without deleting other dates. The base legacy/current cleanup replaces only `unit_statistics_v1` and `annual_cost_legacy_v1` daily rows. The current canonical adapter separately replaces only its selected v2/v3 method within that transaction; inactive v1/v2 financial rows survive
...
php artisan forecasting:report-fixed-contracts  # Read-only stored median evaluation summary
```
Local scheduled generation uses independent five-minute background `--scheduled` ticks from 07:30 through the 12:00 Europe/Helsinki deadline; retail collection starts at 07:15. Evaluation stays 07:45. `MorningFreshness/MorningConsumerExecution` stores per-consumer/date owner UUIDs and terminal outcomes in the existing checkpoints. It shares exclusion with manual writers, never expires running ownership, keeps dependency waiting quiet, and records one deadline alert independently of a live writer. Output transactions check ownership; interrupted or failed work requires inspection. Dry runs never claim or write. Pure option/date validation runs before claims, so invalid manual arguments cannot block a corrected invocation. Same-episode retail republication can extend dates while retaining the original immutable interpretation ID, with source and economic compatibility still required. See `app/Services/MorningFreshness/AGENTS.md` for scope and recovery limits. EEX collection stays unchanged for its other consumers. Forecast freshness requires the full current contract checkpoint, relevant fixed-term observation/publication evidence, current expected-basis unit statistics and publication-before-statistics order. It again requires the same-day EEX checkpoint, current-run prior-date FI Base proof and recent prior-date FI Base data for the futures-adjusted model. Retail-premium freshness keeps exactly the same EEX requirements. Publication-order-only failure can trigger non-dry current statistics overwrite from all active contracts followed by the complete gate recheck; no new deployment-time generation is added.

Important semantics:
- `fixed_term_futures_adjusted_v1` is the local release default. For each 6/12/24-month term, quantile and horizon, add the unrestricted expanding completed same-basis retail-change mean plus the tested fixed-basket seven-day FI futures ridge-1 contribution to current price. Full and feature cohorts independently need max(20, configured minimum) unique starts, targets strictly before issue. Missing futures omits the row. No rolling-window tuning, extra predictors or legacy gap/fair-price calculation. Curves load once per build; feature reuse is build-local.
- Current and history use unit_statistics_v1 only. Current input requires PricingMode's basis on the issue date. Latest ID owns each date before finite validation. Canonical presence through issue, including invalid values, establishes each term's observed-prefix/canonical-continuation boundary. Both pair endpoints must share a basis; reject missing dates, seam crossings and current/future targets. No observed fallback after the boundary; no annual-method gate. Zero and negative finite values remain valid.
- Save full mean, full/feature cohort counts/bounds/basis, feature mean/population std/retail delta mean/slope/contribution, ridge/lag/VAT/policy, current/lag trade dates and basket bounds/coverage. Current retail provenance and transition remain. Confidence uses current-basis full pairs at unchanged 120/365 thresholds, not prediction probability.
- Legacy financial/futures database diagnostics remain null. Real fixed-basket facts live in clearly named feature metadata. Existing nullable columns suffice; this release adds no migration or backfill.
- Every non-current model name is rejected before freshness recovery. Reruns use exact calendar-date identity and skip existing rows unless overwrite is requested; completed rows remain unchanged even with overwrite. Old model rows remain intact.
- Evaluation retains saved basis/unit method/direction threshold, exact-date actuals, legacy defaults, same-basis provenance and unchanged-price error baseline. Completed rows are never relabelled. Dry run and ID-ordered chunks remain. The bounded read-only median report keeps compatible model/horizon/term/basis/method/threshold groups, valid n, unique issue days, named skips, four outcomes and unchanged-direction baseline. No current-statistics refetch or report write.
- Qualified public outlooks use ForecastOutlook saved directions, not consumer_signal timing advice. Visible uncertainty and stored dates/horizon remain. Copy/schema explain contract-price development and wholesale prices in plain Finnish, with one short visible uncertainty notice on the forecast page. Technical fitting details stay in service documentation, not main copy. Legacy financial diagnostics are omitted; crossed quantiles have an explicit notice and are never sorted. The article keeps its ordered-distribution guard; median teasers remain independent.
- Public eligibility/cache identity uses configured model and expected current basis. A switch hides old forecasts until eligible new rows exist. Offered-price history remains separate and complete. Previous gap/research results are historical evidence, not a score for the expanding production variant.
- Recheck environment pins and freshness before release. Git push and a separate exact-context first generation need explicit approval; the model switch hides old forecasts until that generation. No commit/push or production operation occurred. See `../tasks/futures-adjusted-forecast-release/` for the 123-median/two-run isolated replay and independent nine-lane arithmetic checks. Export history starts April 8; production history starts January 21, so exact production values differ. Quantile formula tests do not establish measured quantile accuracy.
...
`phpunit.xml` forces `CONTRACT_STATISTICS_ANNUAL_METHOD_VERSION=annual_cost_legacy_v1`, matching
the config default, alongside the two forced-off pricing flags. AsOf tests opt in through
`config()->set()`. This prevents a local `.env` with production's AsOf method from changing legacy
fixture results. It is test-only and does not change production configuration.

## Frontend behavior

### Public methodology

`resources/views/livewire/about-page.blade.php#menetelma` explains the annual estimation policy.
The simple fixed-price formula divides snt/kWh × kWh by 100 to get euros. Copy distinguishes known
phases, unknown continuations, real-term annualization, Hybrid base-only costs, calendar packages,
audience VAT, and futures with a more uncertain equal day/night price fallback (not equal hourly
customer usage). The visible note directly after the heading explains unequal price certainty,
not a bill promise or a guaranteed cheapest ranking; fully known fixed-price arithmetic stays clear.
Default consumption is flat, not always winter-heavy; 85/15 belongs to basic Time-tariff usage only.
Reset copy describes a retail-price change with wholesale movement from the known period's pricing
context, not a wholesale replacement price or supplier profit. Short-term examples are illustrative;
savings compare the same contract's normal price and stay real-term for short contracts. Retrieval
schedules do not promise freshness. Futures are explained once; EEX covers Spot annual estimates too.
Reuse the existing layout and typed assumptions; do not render an LLM summary. Focused coverage is
in `tests/Feature/AboutPageMethodologyTest.php`, including visible-note order and optional history.
...
- The scheduled fetch runs overnight at 04:00 Europe/Helsinki instead of shortly after evening settlement because FI rows have been observed to lag the evening run; this gives the polite-throttled import more time, but does not guarantee readiness by 07:30. Bounded consumer ticks can wait until noon without rerunning EEX.
...
```
This uses `app/Services/ContractImport/` to import current contracts, refresh `active_contracts`, link high-confidence replacements, and run typed post-import work. Available contracts from a partial postcode acquisition are imported and the command reports the incomplete acquisition. Active rows absent from a partial response are preserved, and replacement linking waits for a complete acquisition. Only the default full scope writes the global `contract_import` freshness checkpoint; postcode-scoped runs never overwrite it. A full run first writes a `running` start marker with a run UUID so a crash cannot preserve an old ready fact; all later checkpoint writes compare that owner. Acquisition/import/genuine required-stage failures are `failed`, partial acquisition is `incomplete`, and a complete successful post-import is `ready` with exact source/target, current active IDs, statistics times and cache proof. Successful initial statistics/invalidation can instead leave `pending_completion` while exact interpretations wait or typed cache conflicts persist. Exact failed targets with nonempty validation errors settle for active and inactive contracts without publication. The cache and current canonical statistics exclude unsafe older publication facts, while forecast and retail publication gates retain their existing scope. `contracts:complete-import` checks this proof each minute, with 60-check/two-hour same-day bounds and durable claims. It never refetches or redispatches. Required statistics lock the checkpoint row inside their existing transaction before any deletion; scoped calls lock an existing same-date row without ever inserting or replacing global readiness. Completion proof is checked at its write boundary only. Later morning readers retain existing forecast scope and publication-only statistics recovery for both new manifests and legacy facts; they do not pin the all-market cache UUID. General/historical statistics are unchanged. EEX start/final checkpoints now also use running status and UUID ownership. See `app/Services/ContractImport/AGENTS.md` for the ownership proof and release approval requirements.
