...
```
Scheduled generation stays 07:30 Europe/Helsinki with `--require-freshness`; evaluation stays 07:45. EEX collection stays unchanged for its other consumers. Forecast freshness requires the full current contract checkpoint, relevant fixed-term observation/publication evidence, current expected-basis unit statistics and publication-before-statistics order. It no longer queries EEX checkpoints or futures presence/age. Retail-premium freshness keeps all EEX checks. Publication-order-only failure can trigger non-dry current statistics overwrite from all active contracts followed by the complete gate recheck; no new deployment-time generation is added.

Important semantics:
- `fixed_term_historical_change_v1` replaces the unreleased v3 gap model as the local release default. Independently for each 6/12/24-month term, quantile and horizon, add the expanding equal-weight mean of all eligible completed same-basis retail changes to the exact current price. Default horizon 30, hard minimum max(20, configured minimum), targets strictly before issue. No futures/hedge/premium/fair-price input or learned coefficients.
...
- Save model/fitting policy, mean, pair/unique-day counts, both endpoint bounds, basis counts, transition and current provenance. Count-based confidence uses accepted current-basis pairs at unchanged 120/365 thresholds, not prediction probability. Minimum default is 20 and the hard floor survives an old 10 pin.
- New obsolete financial diagnostics and futures facts are null. Migration 2026_09_14_000001 loosens the seven existing columns while retaining every old value; its down is deliberately non-destructive/no-op. No new table is needed.
...
- Manager preflight found no model/minimum/threshold/horizon environment pins. Release, automatic nullable migration and a separate exact-context first generation need explicit approval. No commit/push or production operation occurred. See `../tasks/historical-change-forecast-release/` and `app/Services/PriceForecasting/AGENTS.md`.
