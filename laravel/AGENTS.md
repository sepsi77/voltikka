...
- that gate carves out a Hybrid's unquantifiable consumption effect: `calculation.status = unsupported` means "no 12-month total is derivable", not "the base components are unsafe". Without the carve-out the gate closed permanently on all 49 Hybrid contracts and blanked the `hybrid` segment of `/sahkosopimus/tilastot`
