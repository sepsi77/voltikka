...
- `contract_price_annual_costs` — versioned annual-only rows by date, contract, consumption, and method; current canonical collection writes v2 beside retained public v1.
...
- `/sahkosopimus/tilastot` serves cached prepared view data per period + consumption. Its v21 key and source fingerprint include the configured active annual method. Production has used `annual_cost_as_of_v1` since 2026-08-09; this release does not switch it. Current canonical collection writes v2 only. See `../tasks/annual-statistics-correction/rollout.md` for the staged release, backup, reviewed historical apply, separate method-switch approval, and dated v1 rollback. Deployment never applies a historical rebuild
