...
- prepared page entries expire at tomorrow and also bust through `ContractPageCacheVersion`, which includes the import-bumped `ContractListCacheService` version plus cheap source-table aggregates. Annual list, company, and ranking keys include the Helsinki calculation date; their instance memos refresh at midnight or a data-version change, so promotions and annual windows advance without an import
