...
- **History follows the current method.** When the annual-cost calculation method changes, recalculate the stored statistics history with the new method in as-of mode, so that the public series is as complete and continuous as possible. A method change without a history rebuild plan is incomplete work. Only a documented real evidence gap (for example, no FI futures before 2026-04-08) can leave a method transition. No later seller facts/prices, current peers, or future curve evidence are allowed. Approved v3 reconstruction selects the latest validated interpretation of the EXACT covering source, independent of completion relative to the target. Full stored-profile validation uses the exact payload and target date; unsupported date assumptions stay unresolved. Completion and retrospective use remain explicit provenance. The earlier timely/first-later preference was a processing seam defect, not seller evidence; missing exact-source output remains unresolved. No rewrite of observed evidence, retained earlier method rows, and explicit approval for a production apply all stay. Explicit v3 now uses shared Current calculation semantics with dated anchors and exact-date supplier/reset premiums, not current pointers; V5 rules and dated replacement lineage remain unsupported. Local v3 validation covers 242 evidence dates at all three consumptions; July's processing seam is repaired and 11 remaining transitions have documented reference/membership causes. Production release, apply and active-method approval remain separate requirements; defaults and public v2 are unchanged. See `laravel/app/Services/ContractStatistics/AGENTS.md`.
...
`contracts:complete-import` checks proven pending full imports each minute, without refetching Azure or dispatching interpretation work. Existing checkpoint JSON holds exact source/target proof, run ownership, and bounded claims (60 checks, two hours, current Helsinki date only). Required statistics fence ownership under the checkpoint row lock inside their existing date transaction; cache builds stay outside it and retain two-attempt atomic promotion. Pending facts keep morning consumers closed. Exact failed targets with nonempty validation errors are settled even when active, not published. Completion can finish while existing cache and current-statistics guards exclude their unsafe old prices; scoped morning publication gates remain strict. Completion proof is checked at its write boundary; later morning readers retain scoped interpretation checks and publication-only forecast statistics recovery, not a permanent all-market cache UUID gate. Scoped imports never create global readiness checkpoints. Old failed stage-only records cannot resume. See `laravel/app/Services/ContractImport/AGENTS.md`. Releasing this new scheduled production writer and running a fresh full import require separate explicit approval; no automatic import retry or historical repair is added.

## Bounded morning consumers (local implementation)

Retail collection and forecast generation use independent five-minute background `--scheduled` runs from 07:15 and 07:30 through 12:00 Helsinki. All freshness requirements remain. Existing checkpoints hold per-consumer/date ownership, completed outcomes and one deadline alert; waiting is quiet, and failed or interrupted writers require inspection rather than automatic retry. Manual writers share exclusion, dry runs stay read-only, and forecast publication-order recovery remains. Full EEX and contract imports now record `running` at start; final checkpoints compare run ownership. Evaluation and producer retry policy are unchanged. See `laravel/app/Services/MorningFreshness/AGENTS.md` and `tasks/bounded-morning-consumers/`. This is local behavior, not a production release claim.
...
- Low-prominence market-insight pills on comparison heroes reuse cached precomputed price statistics/forecasts; they are informational only and do not affect ranking. The broad pill uses a consumption-independent seller-set energy-price index of national direct General rates: Spot, Time, Season, packages, and Hybrid are outside the overall value; one supplier observation per fixed-term/open-ended/market-reset family prevents duplicate offers from dominating; fixed launch weights combine the families; and its 30-day change requires the exact prior date. Exact 6, 12, and 24 month fixed-term SEO listings keep their matching offered-energy-price statistic segment and eligible forecast duration
...
- Versioned annual-cost persistence uses separate `contract_price_annual_costs` rows so legacy and as-of methods can coexist without rewriting observed unit-price evidence. Historical AsOf calculations use the union of exact-date snapshot and component identities, strict interpretation chronology, exact-target as-of Spot, and supplier-episode calculation. A component-only identity yields three typed `missing_historical_snapshot_identity` exclusions under `unclassified`; snapshot-only canonical evidence stays eligible. Stored rolling-365 Spot levels require the exact target date, finite values, and valid positive hours; an older stored level is never carried forward. New local-window evidence keeps the exact 365-Helsinki-date identity. At least 98% verified coverage permits day/night shape offsets; sparse or legacy evidence can remain a level fallback but not verified local shape. A complete futures strip stays usable with zero offsets at lower confidence, retaining missing/partial historical provenance. Raw timestamps must be exact UTC hours without duplicates or non-finite values. Current canonical daily collection reuses the exact public canonical outcomes and writes `annual_cost_as_of_v2` only, then batch-loads current-pointer provenance without querying `price_components`. Historical v2 uses canonical reset estimates and date-safe supplier seasonal inputs. The old v1 hold branches remain diagnostic, not a reproduction of frozen original mathematics. Both methods use one version-aware AsOf writer, which rejects empty or incomplete apply identity sets before deletion and counts only available contributors in aggregate basis fields. The shared stored compatibility identity is method version + calculation basis + estimate method + estimate basis, without pricing basis, and stays strict for audit. Public aggregate median charts include the actual annual method version in the display key and derive a presentation regime from the dominant estimate method in valid AsOf basis counts; minority-method changes do not split a line, while dominant transitions still do. Weekly/monthly statistics series append the latest daily median at its exact date and show point markers. Daily charts keep the dense lines clean but mark each series' latest non-null point, so a current value after a method gap cannot be invisible. The current write stays inside the required date transaction; feature-off and historical observed runs stay legacy-only. Every public annual reader filters by the configured active method. `annual_cost_as_of_v2` has been active in production since 2026-09-12 after explicit approval. Historical v2 was applied for 233 evidence dates from 2026-01-21 through 2026-09-11 (no evidence date on February 12): 213,755 annual rows and 7,618 aggregates. V1 remains retained at 2026-09-11 for a separately approved dated rollback, with its snapshot joins preserved. Historical snapshots and price components remain unchanged. See `tasks/annual-statistics-v2-rollout/apply-results.md` for proof. V2 writes preserve stored v1 financial rows byte for byte. The annual table has no compatibility-snapshot foreign key, but same-day snapshot replacement can leave provenance IDs stale and remove company date/contract joins. Avoid overwriting the last retained v1 date. Historical correction apply defaults to v2 and also permits an explicit method-scoped v3 target. Partial v3 exact-source reconstruction is implemented, but as-of premium/anchor integration is pending: v3 is not ready for release. Public/current production stays v2. Apply compares the stored baseline before writing and never runs automatically on deployment. A rebuild is nevertheless a required, planned step of each annual method change ("History follows the current method"); the known method seams at 2026-09-21 and their repair are in `tasks/annual-statistics-history-continuity/`. An approved full database backup and separate production approval are required before mutations or an active-method switch. Switching back selects dated retained v1, not new current v1 calculations. See `tasks/annual-statistics-correction/rollout.md`
...
- Forward/current snapshots use typed canonical outcomes for annual totals, unit rates, fees, Spot facts, packages, and measured offer state without reading `price_components`; historical backfill remains observed seller evidence. Both snapshot and aggregate rows carry `pricing_basis`. Current consumers select the latest date for the basis expected by the canonical flag, while older observed rows remain historical evidence. One basis owns each newly calculated date, including stale canonical exclusions. Canonical quarterly resets keep the familiar `quarterly` (`Kvartaalisähkö`) statistics segment. Monthly, seasonal, and other resets use `market_reset` (`Jaksoittain vaihtuva hinta`). Observed history keeps its original text-based keys, with no row rewrite or backward projection. The public page/CSV explain the distinction
...
- Historical reconstruction uses dedicated episode and interpretation tables. `contracts:backfill-historical-interpretations` is read-only by default. It streams deterministic 25-contract discovery chunks into a compact exact-manifest/action plan, verifies the plan hash before any apply write, and repeats discovery in one transaction to persist full audit payloads. Semantic LLM reuse still ignores storage row IDs, while a separate manifest fingerprint binds every target date, snapshot ID, component composite ID, and normalized economic digest. Backcast prose can recover stable classification/mechanism facts, but versioned deterministic validation binds each numeric billed/package fact to one cited exact structured component by canonical type, source unit, amount role, and scoped discount timing. It also requires null recurring-period dates and consumption-effect numbers and forbids `detected` backcast deception. Historical model calls use a 300-second, one-HTTP-attempt policy. One initial call plus two repair calls fits the 1,000-second historical job timeout. The shared Supervisor worker runs `php artisan queue:work --queue=default,historical-interpretation --timeout=1020 --tries=3 --max-time=3600`, and database queue `retry_after` has a code-enforced minimum of 1,050 seconds. Laravel checks the maximum time between jobs, so the current job finishes before Supervisor starts a fresh worker. A lower environment value cannot release the job while it still runs. The versioned AsOf resolver can use one exact-current-fingerprint validated result only when immutable source chronology has no covering observation; older dedicated analyses are ignored and all other source-path failures stay closed. Eligible pre-cutoff episodes can align semantic methods across the rollout, but component-only dates and real evidence/method transitions remain unavailable or chart gaps. The 2026-07-22 cutoff is deliberate because later dates use immutable source chronology. There is no schedule. The public method has been `annual_cost_as_of_v2` since 2026-09-12; v1 remains dated rollback evidence at 2026-09-11 with snapshot joins preserved. See `tasks/annual-statistics-v2-rollout/apply-results.md`

### Canonical phase-aware pricing (deceptive-pricing fix)
- **Approved target policy (2026-09-15), implemented locally, not deployed; default-V4 technically ready, awaiting user deployment approval:** the primary metric is a comparable annualized contract price, not a next-12-month bill promise. Always annualize the six-month term, regardless of continuation or Hybrid classification. Preserve known non-fixed-term prices, then project unknown months with current FI futures and a defensible retail premium from own lineage → comparable company → comparable market evidence. Incomplete promotions must not improve ranking; use one trusted lineage resolver and separate fee changes from energy repricing. Monthly future projections remain accepted estimates. The authoritative [approved annualized comparison policy](laravel/app/Services/CanonicalPricing/AGENTS.md#approved-annualized-comparison-policy-2026-09-15) governs intended future changes where the older estimator/ranking notes below conflict. See [current local status and remaining scope](laravel/app/Services/CanonicalPricing/AGENTS.md#current-local-status) before treating any target rule as implemented. It does not redesign ordinary fully known 12/24-month first-year pricing, imply deployment, or authorize production mutations. Existing billing, VAT, data, cache, and historical-evidence safeguards remain.
- **Current local stages, not deployed:** shared lineage, six-month annualization, promotion guards, unchanged-energy/fee-only handling, and current reset premium fallback are implemented within bounded scopes. Mandatory base-effect Hybrid support reuses the unchanged-energy supplier and known-period reset paths; these earlier bounded stages retain their historical acceptance records. Source-backed actual/normal energy plans, proof-bound current parser use, normal evidence/premium integration and strict public transport are now implemented locally, including `energy_price_guaranteed`. Actual certainty and estimated normal savings remain separate. Real Cheap first-month proof works; five materially limited sources remain Unknown. A later announced 11 holds 11 without a valid new reference, never old 9. See `tasks/source-validated-energy-rules/current-integration-plan.md`, `service-integration.md`, `public-output.md` and `source-language-completion.md`. Current repairs and the fresh strict-transport replay are complete. Independent review verified all 27 numeric changes; the manager accepts policy conformity, not improved forecast accuracy. Full synthetic video/template review and final UI checks pass within their stated limits. Final network-denied PHP and smoke gates pass. The manager accepts default-V4 code as technically ready for user deployment approval; final Pint, context/mirror and source-hash checks all passed. Production has not received these fixes. See `tasks/source-validated-energy-rules/release-readiness.md` for current evidence, limits and pending approval. Original failed reports remain dated historical evidence. Production producer activation is separate: defaults remain V4/v19/v17/parser-v1. See `tasks/source-validated-energy-rules/release-plan.md` for approvals, the coordinated five-key profile switch and compatible rollback limits.
...
- `PricingMode` is one immutable request/command snapshot for canonical state, reset-shift state, expected statistics basis, and cache markers. `CalculatedCostPayloadSchema::VERSION` is the one calculated-cost cache schema; list, company, ranking, and prepared-page caches keep separate outer versions but all include this shared dependency. Current shared schema v19 invalidates calculated-cost semantics without rewriting stored historical statistics or interpretations or switching the stored annual method. Shared annual prices retain the last active generation across midnight and beyond 48 hours. Complete imports with successful required statistics build all eight presets and the default company totals privately, verify every payload, then atomically activate one generation. Immediate interpretation/EEX/manual invalidations remain safety boundaries. Current source/publication identity guards cached canonical facts; relational publication permission never gates canonical listability. Retired tracked payloads receive one hour of reader grace, then a bounded scheduled cleanup physically deletes exact owned keys with durable retry state. This is not a database snapshot. See `laravel/app/Services/Caching/AGENTS.md`
...
- Assigns a deterministic comparability verdict deciding list inclusion and sort key. Known prices stay exact; unknown annual segments use an explicit latest applicable price or disclosed normal continuation. A later known phase always wins. No gap becomes free energy or extended promo savings. Unknown future prices alone do not exclude a contract; unidentifiable or conflicting pricing does. Short fixed terms use real-term costs (including explicit in-term assumptions) annualized for comparison; Hybrids keep every disclosed base phase and exclude the consumption effect. Typed facts generate warnings, never LLM summary prose
- Annual consumption defaults to the same flat calendar-month profile across tariffs; explicit heating/cooling retains its shape. Relative dates, ordinary fee proration, and monthly output use no-overflow contract anniversaries; package fees/allowances stay calendar-month scoped. Inherited one-time fees apply once. Annual equivalent rates reconcile with billed energy and costed consumption
- Canonical VAT convention: Household/Both/legacy-null prices include VAT; Company prices exclude it. Explicit component VAT is normalized in calculation copies; unknown VAT assumes the target basis. Shared inclusive market curves and Spot values convert once for Company, not again after offsets. Source interpretations and stored historical evidence are not rewritten
...
- **Current estimator scope:** v1/v2 retain their separate Historical identity/reference rules. Explicit annual v3 uses shared Current semantics with dated exact-contract anchors and exact-date premiums, not current pointers or lineage links. Remaining differences require date safety or missing evidence; this is not full-store amount parity. Current guards finite prior-vintage evidence, reports effective hold beta 0, uses the selected full-profile seasonal anchor and discloses only actually billed model floors. Ambiguous known-short duration ranges fail closed; fee-only short terms require in-term energy proof. `RESET_FORWARD_SHIFT_ENABLED` remains reset-only, and legitimate negative Spot remains allowed. See the closest estimator contexts for details.
- **Market-reset annualised pricing** (`laravel/app/Services/CanonicalPricing/MarketReset/`): market-reset products (monthly/quarterly/seasonal/other repricing, e.g. Kokkolan Tyyni, Helen Markkinahintasähkö, kvartaalisähkö, Lumme Perussähkö) used to be annualised by holding the **current period's seasonal price** flat for twelve months, which understated them badly in summer and overstated them in winter across roughly 32 lineages. They are now annualised with an FI forward-curve shift, `P_m = P_current + beta * (F_m - F_reference)`: the current period stays exact and only the tail is repriced. Cadence `other` stays an estimate and uses the quarterly calendar and reference proxy because the seller does not publish exact reset boundaries. **Two vintages on purpose** — `F_m` reads today's curve because the coming year's level is what the customer pays, while `F_reference` reads the **pricing** vintage (latest `trade_date` before the current period started) because that is the forward the seller priced from; reading it at today's vintage inflates the implied spread by pure front-month convergence (measured 1.58 c/kWh, about +79 €/yr at 5000 kWh). Current local ladder: forward curve with a valid own reference, or comparable premium fallback when that reference is missing → multi-year spot seasonal index (lower confidence) → hold flat. V1/v2 Historical retain their old reference/fallback rules. Explicit annual v3 uses the Current ladder with dated reset premiums and never reads current peers; see `MarketReset/AGENTS.md` for the bounded, not-deployed integration. **Gated behind its own `RESET_FORWARD_SHIFT_ENABLED` (default off)** because `CANONICAL_PRICING_ENABLED` is already true in production; the flag varies the list/ranking/page cache keys. Staged with `php artisan contracts:compare-canonical-pricing --resets`. Cards and detail pages show the known current-period price and the estimated 12-month equivalent as two separate figures; there is deliberately **no** deceptive-pricing label, because a published reset mechanism is not hidden promotional text
- **Supplier-adjusted open-ended annual pricing** (`laravel/app/Services/CanonicalPricing/SupplierAdjusted/`): the current local path shares unchanged-energy `General`/`Time`/`Season` extraction across redundant phases and fully disclosed fee-only changes for eligible `OpenEnded` contracts. It supports ordinary `FixedPrice` and mandatory base-effect Hybrid contracts, with separate comparable premium families and the effect excluded. V1/v2 Historical keep the strict one-phase ordinary path; explicit annual v3 uses Current candidate semantics on dated evidence. Source-proved energy-changing promotion/future plans now use the paired rule path and existing estimators; see `tasks/source-validated-energy-rules/service-integration.md`. Packages, Spot margins and recurring resets do not enter this supplier path. See `SupplierAdjusted/AGENTS.md` for exact eligibility and evidence limits. The current calendar-month remainder stays at the published rate. Later months use an FI forward shift anchored to the start of the current observed seller-price episode, then fall back to a Spot seasonal index and hold-current. Time and Season tariffs apply the same additive monthly shift to each exact current rate. Statistics snapshot weights serve episode matching only; the displayed 12-month equivalent uses billed energy and costed consumption. Episode matching follows date-ordered evidence across bases, with observed precedence only on the same date; gaps, unknowns, conflicts, and a newer rate change break the run. All rungs remain typed estimates in `supplier_adjusted_estimate`; exact bill-period pricing stays factual. Multiple monthly-fee variants use the calculator's existing conservative maximum rule; this covers both Sulaketariffi's five identical €4.20 components and fuse-size fee variants without changing the exact current fee used for ranking. Cards and ContractDetail separate published current rates from the estimated annual equivalent and show the shared Arvio popover for every fallback rung, without claiming a reset cadence. Ordinary contracts retain Kiinteä hinta; Hybrid copy also states that the consumption effect is excluded
- **Forward-looking Spot annual pricing** (`laravel/app/Services/CanonicalPricing/SpotForward/`): canonical Spot rankings use the FI Base forward curve normalized to the audience VAT basis for every month touched by the coming 12-month window. The in-delivery month uses its last pre-delivery curve; later months use the latest curve before the comparison date. The rolling-365 overall/day/night evidence supplies only an additive intraday load-shape adjustment. The exact contract margin, fees, phases, and measured discounts are then applied. Missing, stale, or incomplete curve evidence falls back as one typed `rolling_365_fallback` when historical levels exist; it never mixes forward and historical months. A complete curve with insufficient historical shape stays available at lower confidence with explicit zero day/night offsets. New `rolling_30d_local` / `rolling_365d_local` records use Helsinki date windows and explicit UTC-hour parsing; readers accept legacy types, prefer local on same-date ties, and retain legacy/coverage provenance without rewriting old records. `spot_estimate` records both curve vintages, monthly prices, shape evidence, confidence, and fallback flags. Exact-period bill comparison still uses realized hourly Spot prices and never the annual projection
...
- Command `retail-premiums:collect` (bounded five-minute background ticks from 07:15 through 12:00 Europe/Helsinki); the scheduled form uses `--scheduled` and defers unless the same-day full contract and EEX checkpoints, exactly one pointed source-observation episode and current publication for every active contract, current-run prior-date FI Base proof, and recent FI Base database data are ready. Historical/manual collection remains opt-in and compatible. Read-only diagnostic: `retail-premiums:cross-check`
...
- The approved local release model is `fixed_term_futures_adjusted_v1`: current price plus the unrestricted expanding mean of completed same-basis retail changes, adjusted by the tested seven-calendar-day fixed-delivery-basket FI Base futures feature with fixed ridge 1. Fit term 6/12/24, p20/median/p80 and horizon independently. Default horizon 30; full and feature cohorts each need max(20, configured minimum) unique starts with targets strictly before issue. Missing futures omits the forecast, never falls back under the new identity. No rolling-window tuning or extra predictors. See `tasks/futures-adjusted-forecast-release/`.
...
- Confidence uses accepted current-basis full pairs only, at unchanged 120/365 thresholds. It describes history coverage, not accuracy. Legacy financial/futures columns stay null; real fixed-basket diagnostics and fitted parameters are saved as feature metadata. No new migration is required. Old forecasts and completed evaluations remain intact and evaluable from saved provenance. Forecast freshness again requires current EEX checkpoint/current-run prior-FI proof and recent prior-date FI Base data; retail-premium checks and statistics recovery rules stay unchanged.
...
- **Local schedule** uses five-minute background generation ticks with `--scheduled` from 07:30 through 12:00 and unchanged 07:45 evaluation, Europe/Helsinki. Publication-order-only recovery can overwrite current statistics and recheck the full gate on a non-dry run; missing EEX readiness blocks recovery. No deployment-time generation is added.
- Every other generation model pin is rejected before recovery writes. Model/horizon cache identities isolate the new rows. Switching hides old public forecasts until new generation: this is a release gap. Normal Git deployment and a separate exact-context first generation require explicit approval after fresh preflight. No production mutation, commit or push occurred in this local task.
- Isolated replay matches 123 frozen primary median predictions and all nine September 13 lanes against an independent formula. Quantile arithmetic verification is not measured quantile accuracy. Production has January 21 onward retail history; the export begins April 8, so exact production values differ. See `laravel/app/Services/PriceForecasting/AGENTS.md` and `tasks/futures-adjusted-forecast-release/` for policy, replay and release plan.
...
- **`FixedPrice` (`kiintea-hinta`) and `GeneralElectricity` (`yleissahko`)** retain the SQL constraint `pricing_model = FixedPrice` **AND** `canonical_calculation->status = 'exact'`. A plain source-enum filter would also admit quarterly/monthly market resets. This constraint proves neither a constant first-year/whole-contract energy price nor a source-backed guarantee: disclosed phased prices and eligible OpenEnded contracts can pass. Copy explains known price phases and possible OpenEnded changes; GeneralElectricity means one rate across clock times, not an unchanged annual price. Do not infer a guarantee from `exact` or weaken the retained SQL constraint.
