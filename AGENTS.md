...
- **Schedule**: daily forecast run at 07:30 and evaluation at 07:45 Europe/Helsinki. The scheduled generation command uses `--require-freshness`; it defers on missing same-day full import checkpoints, incomplete active pointed-episode/publication coverage, statistics that started before a required publication, no current fixed-term 6/12/24 statistic in the expected pricing basis, missing current-run prior-date FI proof, or stale FI Base database data
