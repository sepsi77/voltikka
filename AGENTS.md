...
- **KNOWN DEFECT, fix specified and pending**: market-reset products (monthly/quarterly repricing, e.g. Kokkolan Tyyni, Helen Markkinahintasähkö, kvartaalisähkö) are annualised by holding the **current period's seasonal price** flat for twelve months. That understates them badly in summer and overstates them in winter — measured at **−308 €/yr** at 5000 kWh for Tyyni on 2026-07-24 (279 € shown against 588 € correct), across roughly 32 lineages. The fix is a shape-only forward-curve shift using today's FI EEX curve; it needs no historical futures. Spec and measurements: `tasks/market-reset-annualised-pricing/`
- **TO BE IMPLEMENTED IN THE FUTURE**: per-company calibration of that estimate (the reference period each seller prices from, and the pass-through coefficient). Blocked on data that cannot be bought back — the FI futures curve only exists from 2026-04-08 and EEX serves an approximately 45-day rolling window, so earlier vintages are permanently gone. Becomes possible for quarterly products after the **1 October 2026** resets. The first rollout therefore uses one global coefficient.
